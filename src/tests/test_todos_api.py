...
def test_get_todos(client, mocker):
    ...
    # desc
    response = client.get("/todos?order=desc", headers=header)
    assert response.status_code == 200
    assert response.json() == {
        "todos": [
            {"id": 2, "contents": "test", "is_done": False},
...
def test_get_todo(client, mocker):
    ...
    mocker.patch.object(
        ToDoRepository,
        "get_todo_by_todo_id",
        return_value=ToDo(id=1, contents="test", is_done=True),
    )
...
def test_create_todo(client, mocker):
    ...
    mocker.patch.object(
        ToDoRepository,
        "create_todo",
        return_value=ToDo(id=1, contents="test11", is_done=False),
    )
...
def test_update_todo(client, mocker):
    ...
    mocker.patch.object(
        ToDoRepository,
        "update_todo",
        return_value=ToDo(id=1, contents="test", is_done=False),
    )
